...
namespace Massive
{
    ...
    public partial class DynamicModel
    {
        ...
        /// <returns>Streaming enumerable with expandos, one for each row read</returns>
        public IEnumerable<dynamic> QueryAndLink(string sql, Action<dynamic, IList<dynamic>> mapper, bool useSplitter, params object[] args)
        {
            ...
            int objectCount = linkedObjectsCount + 1;       // Number of objects in each record;
            object[] objects = new object[objectCount];     // List of the objects that are read from each record.
            int objectIndex;                                // Index of the last item in "objects".
            ...
            using (DbConnection con = OpenConnection())
            {
                ...
                using (DbDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    {
                        // Resetting stuffs.
                        Array.Clear(objects, 0, objectCount);
                        objectIndex = 0;
                        ...
                        for (int i = 0; i < reader.FieldCount; i++)
                        {
                            ...
                            if (i == 0 || (!useSplitter && fieldName.Equals("id", StringComparison.OrdinalIgnoreCase)) || (useSplitter && fieldName.StartsWith(SPLITTER_COLUMN)))
                            {
                                /* An "Id" or splitter column is found or it is the first column that was retrived (meaning it is the begining
                                 * of the first object in the record). Columns must be mapped to a new object from now on. */
                                obj = new ExpandoObject();
                                objects[objectIndex++] = obj;
                                if (useSplitter && fieldName == SPLITTER_COLUMN) continue;
                            }
                        ...
                        mapper.Invoke(objects[0], new ArraySegment<object>(objects, 1, linkedObjectsCount));
        ...
        /// <summary>
        /// Statically-typed version of <seealso cref="QueryAndLink(string, Action{dynamic, IList{dynamic}}, bool, object[])"/> method.
        /// </summary>
        /// <returns>Streaming enumerable with objects of type T, one for each row read</returns>
        public IEnumerable<T> QueryAndLink<T>(string sql, Action<T, IList<object>> mapper, Type[] linkedTypes, bool useSplitter, params object[] args) where T : new()
        {
                    ...
                    while (reader.Read())
                    {
                        ...
                        for (int i = 0; i < reader.FieldCount; i++)
                        {
                            ...
                            if (i == 0 || (!useSplitter && fieldName.Equals("id", StringComparison.OrdinalIgnoreCase)) || (useSplitter && fieldName.StartsWith(SPLITTER_COLUMN)))
                            {
                                ...
                                if (i == 0)
                                {
                                    // It its the first (main) object in the record.
                                    objectType = typeof(T);
                                    obj = new T();
                                }
                                else
                                {
                                    // It is a linked object.
                        ...
                        mapper.Invoke((T)objects[0], new ArraySegment<object>(objects, 1, linkedObjectsCount));
                        yield return (T)objects[0];
                    }
                }
            }
        }
